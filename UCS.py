...
def get_successors(state, grid):
    ...
    for action, (dr, dc) in DIRECTIONS.items():
        ...
        if 0 <= nr < rows and 0 <= nc < cols and grid[nr][nc] != '#':
            ...
            successors.append((1, State((nr, nc), state.dirty, state.path + [action])))
        # Vaccum action 
    if (r, c) in state.dirty:
        new_dirty = set(state.dirty)
        new_dirty.remove((r, c))
        successors.append((1, State((r, c), new_dirty, state.path + ['Clean'])))

    return  successors
def ucs(initial_state, grid):
    frontier = []
    ...
    while frontier:
        ...
        for step_cost, succ in get_successors(state, grid):
            heapq.heappush(frontier, (cost + step_cost, succ))
            nodes_generated += 1
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 USC.py uniform-cost <world_file>")
        return
    ...
    if algorithm == 'uniform-cost':
        path, generated, expanded = ucs(initial_state, grid)
    else:
        print("Only uniform-cost is implemented.")
        return
